...
```bash
bun install
bun link    # Makes the 'agentbridge' and 'abg' commands available globally
```
...
```bash
abg dev     # Register local marketplace + sync plugin
abg claude  # Start Claude Code with plugin auto-loaded
```

`abg` is a short alias for `agentbridge`; the two are interchangeable.

After changing plugin or runtime code, run `abg dev` again and restart Claude Code (or `/reload-plugins` in an active session).
